...
export default async function EventDetailPage({ params }: Params) {
  ...
  const event = await client.fetch(`*[_type == "ourEvents" && slug.current == $slug][0]{
    title, category, description, "image": image.asset->url, "gallery": gallery[].asset->url
      ...
      <section className="py-20 px-6 md:py-32 bg-slate-50 flex-grow">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
            {event.gallery?.map((imgSrc: string, index: number) => (
              <div 
                key={index} 
