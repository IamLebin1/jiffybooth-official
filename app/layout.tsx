    ...
    <html lang="en" className={`${inter.variable} scroll-smooth`}>
      <body className={`${inter.className} antialiased bg-jiffy-cream min-h-screen flex flex-col`}>
        ...
        <div className="fixed bottom-8 left-8 z-50 flex flex-col items-start gap-3">
          {/* Message Box */}
          {showWhatsAppMessage && (
            <div className="relative bg-jiffy-dark text-white px-6 py-4 rounded-2xl shadow-xl max-w-[280px] animate-in slide-in-from-bottom duration-300">
              <button 
                onClick={() => setShowWhatsAppMessage(false)}
                className="absolute -top-2 -right-2 bg-white text-jiffy-dark rounded-full w-6 h-6 flex items-center justify-center hover:bg-gray-100 transition-colors shadow-md"
              ...
              <div className="absolute -bottom-2 left-8 w-0 h-0 border-l-[10px] border-l-transparent border-r-[10px] border-r-transparent border-t-[10px] border-t-jiffy-dark"></div>
            </div>
          )}
          ...
          <a 
            ...
            className="bg-whatsapp p-4 rounded-full shadow-lg hover:scale-110 active:scale-95 transition-transform flex items-center justify-center"
          >
            <svg viewBox="0 0 24 24" fill="white" className="w-[45px] h-[45px] md:w-[50px] md:h-[50px]">
