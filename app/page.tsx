...
type EventCategory = { title: string; image?: string; description?: string; count: number };
...
export default function Home() {
      ...
      try {
        const data = await client.fetch(`{
            "mainPage": *[_type == "mainPage"] | order(_updatedAt desc)[0] {
              ...
              "brands": brands[] { name, "logo": logo.asset->url }
            },
            "services": *[_type == "ourServices"] | order(order asc) { title, description, "slug": slug.current, "image": image.asset->url },
            "events": *[_type == "ourEvents"] | order(_createdAt asc) { title, category, description, "slug": slug.current, "image": image.asset->url }
        ...
        // keep non-fatal: log for dev inspection
  ...
  // --- 2. GLIDE / TEMPLATES SETUP ---
  useEffect(() => {
    if (!glideRef.current || !pageData?.templates || pageData.templates.length === 0) return;

    const glideEl = glideRef.current as unknown as HTMLElement;
    const glide = new Glide(glideEl, {
      type: 'carousel',
      gap: 48,
      perView: Math.min(3, pageData.templates.length),
    ...
    const startAutoplay = () => {
      ...
      autoplayTimer = setInterval(() => glide.go('>'), 4000);
    };

    glide.on('run.before', (ctx?: unknown) => {
      const move = ctx as { direction?: string } | undefined;
      if (move?.direction === '<' || move?.direction === '>') startAutoplay();
    });
  ...
  // Derive the brands, services and grouped event categories for previews
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="md:hidden -mx-6 px-6 overflow-x-auto scrollbar-hide pb-2">
            <div className="flex gap-4 w-max snap-x snap-mandatory">
              {filteredPreviewEvents.slice(0, 8).map((item: EventCategory, index: number) => (
                <Link
                ...
                >
                  ...
                  <h3 className="mt-3 font-serif italic text-lg leading-tight">{item?.title || 'Untitled Category'}</h3>
                </Link>
              ))}
            </div>
          </div>
            ...
            {filteredPreviewEvents.slice(0, 8).map((item: EventCategory, index: number) => (
              ...
              >
                <div className="relative overflow-hidden rounded-[1.7rem] aspect-[1/1] bg-stone-200">
                  ...
                  <h3 className="mt-3 font-serif italic text-xl md:text-2xl leading-tight">{item?.title || 'Untitled Category'}</h3>
                </div>
              </Link>
            ))}
          </div>
